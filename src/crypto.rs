...
//         pqc_kyber::encapsulate(&hex::decode(public_key).unwrap(), &mut rng).unwrap();
...
//     let shared_secret_bob = pqc_kyber::decapsulate(
...
//     Vec::from([
//         hex::encode(keys.public.bytes),
//         hex::encode(keys.secret.bytes),
//     ])
// }
...
//     };
//     public.verify(msg.as_bytes(), hex::decode(sig).unwrap().as_slice())
// }
...
//     String::from_utf8(BASE64_STANDARD.decode(ciphertext.as_bytes()).unwrap()).unwrap()
// }
...
//         base64_encode(hmac_encrypt(
//             format!("{}.{}", base64_encode(version), base64_encode(data)),
...
//     use super::{
//         aes_decrypt, aes_encrypt, dsa_pair, dsa_sign, dsa_verify, hmac_encrypt, hmac_verify,
//         kem_decapsulate_private, kem_encapsulate_public, kem_pair, sha512_encrypt,
//     };
...
//             ciphertext_secret1.get(0).unwrap().to_string(),
//             public_private.get(1).unwrap().to_string(),
//         );
//         assert_eq!(ciphertext_secret1.get(1).unwrap().to_string(), secret2);
//         println!("{}", secret2);
//     }
...
//         let is_verified = dsa_verify(message, signature, keys.get(0).unwrap().to_string());
...
//         assert_eq!(
//             aes_decrypt(
//                 aes_encrypt("hello".to_string(), "hello_code".to_string()),
